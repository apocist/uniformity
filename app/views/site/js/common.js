define(['jquery'], function($) {
	 ...
	 */
	$.loadScript = function (url, callback, callbackfail) {
		jQuery.ajax({
			url: url,
			cache: true,
			dataType: 'script',
	 ...
	 * @param callbackfail action to perform is script failed to load
	 */
	$.loadScripts = function (urls, callback, callbackfail) {
		var url = urls.shift();
		if (urls.length > 0) {
	 ...
	 */
	$.loadScriptsIfNeeded = function (boolean, url, callback, callbackfail) {
